...
def pso_findposition(target_matrix, sensor_matrix):
    # 要求单个目标被几架无人机覆盖
    ...
    for numofcycle in range(numofcy):
        ...
        # save_result(x_best, idcn, uavcn, ccn, target_num, sensor_num, ccn_sum, cn, fitness, run_time_temp, numofcycle,
        #             N, T)
        # print('------------------------------------------------------------------')
        # print('scid = \n', scid)
        print('numofcycle = ', numofcycle)
    ...
    # test_chart(idcn_numofcycle_matrix[:, ccn_sum_max_i], uavcn_numofcycle_matrix[:, ccn_sum_max_i], sensor_num,
    #            target_num)
    #
    # plot_map(x_bestx, x_besty, sensor_num, target_num,target_matrix,sensor_matrix)

    return x_bestx, x_besty
